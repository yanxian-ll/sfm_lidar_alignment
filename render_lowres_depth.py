...
def read_view_list_txt(view_list_txt: str):
    if view_list_txt is None:
        return None
    ...
    if len(names) == 0:
        raise ValueError(f"视角名单为空: {path}")
...
def main():
    ...
    parser.add_argument("--max-image-edge", type=int, default=512, help="图像最大边；>0 时对相机分辨率和内参做等比例缩放，并保存到输出目录/cams")

    parser.add_argument("--point-size", type=float, default=1.0, help="Open3D 点大小")
    ...
    selected_names = read_view_list_txt(args.view_list_txt)
...
python render_lowres_depth.py nanfang/models/pc/0 \
    ./nanfang ./nanfang_render_lowres \
    --save-depth-npy --save-depth-vis \
    --point-size 0.8

python render_lowres_depth.py yanghaitang/models/pc/0 \
    ./yanghaitang ./yanghaitang_render_lowres \
    --save-depth-npy --save-depth-vis \
    --point-size 0.8

python render_lowres_depth.py xiaoxiang/models/pc/0 \
    ./xiaoxiang ./xiaoxiang_render_lowres \
    --save-depth-npy --save-depth-vis \
    --point-size 0.8
